...
import com.edu.hust.Utils.MathUtil;
import com.edu.hust.Utils.StringUtils;
import com.edu.hust.word.similarity.ISimilarity;
import com.edu.hust.word.similarity.Similarity;
import com.edu.hust.word.similarity.hownet.IHownetMeta;
import com.edu.hust.word.similarity.hownet.sememe.SememeParser;
 ...
 * @author xuming
 */
public abstract class ConceptParser implements IHownetMeta, ISimilarity, Serializable {
    private static final long serialVersionUID = -7353820725833608159L;
    ...
    private static void loadFile() throws IOException {
        ...
        InputStream inputStream = new GZIPInputStream(ConceptParser.class.getResourceAsStream("/" + path));
        load(inputStream);
    }
    ...
    public boolean isConcept(String word) {
        return StringUtils.isNotBlank(CONCEPTS.get(word));
    }
    ...
    public Collection<Concept> getConcepts(String key) {
        Collection<Concept> concepts = CONCEPTS.get(key);
        return concepts;
    }
    ...
    private double getSimilarityAVG(String[] sememes1, String[] sememes2) {
        double similarity;
        double scoreArray[][];
        if (StringUtils.isBlank(sememes1) || StringUtils.isBlank(sememes2)) {
            if (StringUtils.isBlank(sememes1) && StringUtils.isBlank(sememes2)) {
                return 1.0;
            } else {

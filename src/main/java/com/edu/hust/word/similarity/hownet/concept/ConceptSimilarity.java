...
import com.edu.hust.word.similarity.hownet.sememe.SememeSimilarity;
 ...
 * 注意避免分布式环境下递归层级过深
 */
public class ConceptSimilarity extends ConceptParser implements Serializable {

    private static final long serialVersionUID = -266531298748774950L;
    ...
    public ConceptSimilarity() throws IOException {
    ...
    public double getSimilarity(String word1, String word2) {
        ...
        if (StringUtils.isBlank(concepts1)) {
            concepts1 = autoCombineConcepts(word1, null);
        }

        Collection<Concept> concepts2 = super.getConcepts(word2);
        if (StringUtils.isBlank(concepts2)) {
            concepts2 = autoCombineConcepts(word1, null);
        }

        // 未登录词需要计算组合概念
        if (StringUtils.isBlank(concepts1) && StringUtils.isNotBlank(concepts2)) {
            concepts1 = autoCombineConcepts(word1, concepts2);
        }
        if (StringUtils.isBlank(concepts2) && StringUtils.isNotBlank(concepts1)) {
            concepts2 = autoCombineConcepts(word2, concepts1);
        }
        if (StringUtils.isBlank(concepts1) && StringUtils.isBlank(concepts2)) {
            concepts1 = autoCombineConcepts(word1, concepts2);
            concepts2 = autoCombineConcepts(word2, concepts1);
            // 修正
            concepts1 = autoCombineConcepts(word1, concepts2);
...
//    private Collection<Concept> tmp_concepts = null;
//    public void getConcepts(String key) {
//        tmp_concepts = super.getConcepts(key);
//        if (StringUtils.isBlank(tmp_concepts)) {
//            tmp_concepts = autoCombineConcepts(key, null);
//        }
//    }
...
//    }
    ...
    public Collection<Concept> autoCombineConcepts(String newWord, Collection<Concept> refConcepts) {
        ...
        for (String conceptWord : SegmentNewWord(newWord, 3)) {
            Collection<Concept> concepts = super.getConcepts(conceptWord);
            if (StringUtils.isBlank(conceptWord)) {
                concepts = autoCombineConcepts(conceptWord, null);
            }
            ...
            for (Concept head : concepts) {
                for (Concept tail : newConcepts) {
                    if (StringUtils.isNotBlank(refConcepts)) {
    ...
    private List<String> SegmentNewWord(String newWord, int top) {
        ...
        while (word != null && !word.equals("")) {
            String token = word;
            while (token.length() > 1 && StringUtils.isBlank(super.getConcepts(token))) {
